...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div class="border border-gray-300 rounded-lg p-6 bg-gray-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div><strong>Family Status:</strong> {{ str_replace('-', ' ', ucfirst($existingApplication->family_status)) }}</div>
            <div><strong>Application Date:</strong> {{ \Carbon\Carbon::parse($existingApplication->application_date)->format('d M Y') }}</div>
        </div>
    ...
    <form action="{{ route('seat-application.submit') }}" method="POST" enctype="multipart/form-data" id="applicationForm" class="space-y-8">
        ...
        <div class="border border-gray-300 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="university_id" class="block text-sm font-medium text-gray-700 mb-1">University ID (Roll Number)</label>
                ...
                <div>
                    <label for="guardian_mobile" class="block text-sm font-medium text-gray-700 mb-1">Guardian's Mobile Number</label>
                    <input type="tel" name="guardian_mobile" id="guardian_mobile" value="{{ old('guardian_mobile') }}" required
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="md:col-span-2">
                    <label for="guardian_relationship" class="block text-sm font-medium text-gray-700 mb-1">Relationship with Student</label>
        ...
        <div class="border border-gray-300 rounded-lg p-6">
            ...
            <div class="mb-4">
                ...
                <div class="flex flex-wrap gap-4">
                    @php
                    $programs = ['bachelor' => 'Bachelor', 'masters' => 'Masters', 'mphil' => 'M.Phil', 'phd' => 'Ph.D'];
                    $oldProgram = old('program');
                    @endphp
                    @foreach ($programs as $value => $label)
                    <label class="flex items-center">
                        <input type="radio" name="program" value="{{ $value }}" {{ $oldProgram === $value ? 'checked' : '' }} class="mr-2">
            ...
            <div class="mb-4">
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex items-center">
                        <input type="radio" name="physical_condition" value="{{ $value }}" {{ $oldPhysical === $value ? 'checked' : '' }} class="mr-2">
                        <span class="text-sm">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    ...
                    <label class="flex items-center">
                        <input type="radio" name="family_status" value="{{ $value }}" {{ $oldFamily === $value ? 'checked' : '' }} class="mr-2">
                        <span class="text-sm">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="activities[]" value="{{ $value }}" {{ in_array($value, $oldActivities) ? 'checked' : '' }} class="mr-2">
                        <span class="text-sm">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            ...
            <div class="mb-4">
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="other_info[]" value="{{ $value }}" {{ in_array($value, $oldOtherInfo) ? 'checked' : '' }} class="mr-2">
                        <span class="text-sm">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
        ...
        <div class="border border-gray-300 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="file" name="{{ $name }}" id="{{ $name }}" accept=".pdf,.jpg,.jpeg,.png" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                @endforeach

                <div class="space-y-4">
                    @foreach (array_slice($fileFields, 4) as $name => $label)
                    @if($name !== 'signature') {{-- Signature input separate below --}}
                    <div>
                        <label for="{{ $name }}" class="block text-sm font-medium text-gray-700 mb-2">{{ $label }}</label>
                        <input type="file" name="{{ $name }}" id="{{ $name }}" accept=".pdf,.jpg,.jpeg,.png" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
        ...
        <div class="border border-gray-300 rounded-lg p-6 mt-4">
            <label for="signature" class="block text-sm font-medium text-gray-700 mb-1">Student's Signature</label>
        ...
        <div class="border border-gray-300 rounded-lg p-6 bg-blue-50 mt-6">
            ...
            <div class="space-y-3">
                <div class="flex items-start">
                    ...
                    <label for="declaration_info_correct" class="text-sm text-gray-700">All the information provided above is correct</label>
                </div>
                <div class="flex items-start">
                    <input type="checkbox" id="declaration_will_stay" name="declaration_will_stay" required
                        class="mr-3 mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <label for="declaration_will_stay" class="text-sm text-gray-700">I will occupy any type of seat allocated to me in the hall</label>
                </div>
                <div class="flex items-start">
                    <input type="checkbox" id="declaration_seven_days" name="declaration_seven_days" required
                        class="mr-3 mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <label for="declaration_seven_days" class="text-sm text-gray-700">If I do not occupy the hall within seven days of seat allocation, I will never apply for a seat again</label>
                </div>
            </div>
        </div>
