            ...
            <div class="mb-8">
                <nav class="flex items-center space-x-2 text-sm text-gray-600 mb-4">
                    <a href="{{ route('admin.complaints') }}"
                        class="hover:text-blue-600 transition-colors duration-200">Complaints</a>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden complaint-card">
                            ...
                            <div class="bg-gradient-to-r from-blue-600 to-purple-700 px-6 py-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <h2 class="text-xl font-semibold text-white">Complaint
                                            #{{ $complaint->complaint_id }}</h2>
                                    ...
                                    <div class="flex items-center p-4 bg-gray-50 rounded-lg">
                                        <div
                                            ...
                                            @if ($complaint->student && $complaint->student->profile_image)
                                                <img src="{{ asset('storage/' . $complaint->student->profile_image) }}"
                                                    alt="{{ $complaint->student->name }}'s profile image"
                                                    class="w-full h-full object-cover">
                                            @else
                                                <span
                                                    class="text-gray-600 font-medium text-lg">{{ $complaint->student ? substr($complaint->student->name, 0, 1) : 'N' }}</span>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                            <p class="text-lg font-medium text-gray-900">
                                                {{ $complaint->student->name ?? 'N/A' }}</p>
                                            <p class="text-sm text-gray-600">
                                                {{ $complaint->student->student_id ?? 'Unknown ID' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    <div class="space-y-4">
                                        ...
                                        <div class="detail-item">
                                            ...
                                            <span
                                                class="inline-flex px-3 py-1 text-sm font-medium bg-gray-100 text-gray-800 rounded-full">
                                    ...
                                    <div class="space-y-4">
                                        <div class="detail-item">
                                            ...
                                            <div>
                                                <p class="text-gray-900 font-medium">
                                                    {{ $complaint->submission_date->format('M d, Y') }}</p>
                                                <p class="text-sm text-gray-500">
                                                    {{ $complaint->submission_date->format('h:i A') }}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="detail-item">
                                            ...
                                            <p class="text-gray-900 font-medium">
                                                {{ $complaint->submission_date->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 update-form">
                            ...
                            <form action="{{ route('admin.complaint.update_status', $complaint->complaint_id) }}"
                                method="POST" id="statusForm">
                                @csrf
                                <div class="mb-4">
                                    <label for="status" class="block text-sm font-medium text-gray-700 mb-2">New
                                        Status</label>
                                    <select name="status" id="status"
                                        ...
                                        <option value="in_progress"
                                            {{ $complaint->status === 'in_progress' ? 'selected' : '' }}>In Progress
                                        </option>
                                ...
                                <div class="mb-6">
                                    <label for="admin_comment" class="block text-sm font-medium text-gray-700 mb-2">Admin
                                        Comment</label>
                                    <textarea name="admin_comment" id="admin_comment" rows="4" placeholder="Add your response or comment here..."
