...
<head>
    <title>Renewal Applications Report</title>
    ...
    <style>
        ...
        .applications-table td:nth-child(10) {
            text-align: center;
        }
...
<body>
    ...
    <div class="document-title">
        <h2>Seat Renewal Applications Report</h2>
    ...
    <div class="report-info">
        <div class="left">
            ...
            <div>{{ $renewalApplications->count() }}</div>
        </div>
        <div class="right">
            <div class="label">Report Type:</div>
            <div>Renewal Applications - {{ ucfirst($status) }}</div>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 9px;">
        ...
        <tr style="background-color: #2c3e50; color: white; font-weight: bold;">
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 5%;">S.NO</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 6%;">PHOTO</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 20%;">STUDENT DETAILS</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 12%;">SESSION YEAR</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 15%;">SEAT NUMBER</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 10%;">DEPARTMENT</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 8%;">SEMESTERS</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 8%;">CGPA</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 10%;">STATUS</td>
            <td style="padding: 10px; border: 1px solid #000; text-align: center; width: 6%;">DATE</td>
        </tr>
        @foreach ($renewalApplications as $index => $application)
            <tr style="background-color: {{ $index % 2 == 0 ? '#ffffff' : '#f8f9fa' }};">
                <!-- Serial Number -->
                <td
                    style="padding: 8px; border: 1px solid #000; text-align: center; font-weight: bold; font-size: 9px;">
                    {{ $index + 1 }}
                </td>
                ...
                <td style="padding: 8px; border: 1px solid #000;">
                    ...
                    <div style="font-size: 8px; color: #666;">
                        App: #{{ $application->renewal_id }}
                    </div>
                </td>

                <!-- Session Year -->
                <td
                    style="padding: 8px; border: 1px solid #000; text-align: center; font-size: 9px; font-weight: bold;">
                    {{ $application->student->session_year ?? 'N/A' }}
                </td>

                <!-- Seat Number -->
                <td style="padding: 8px; border: 1px solid #000; text-align: center; font-size: 9px;">
                    <div style="font-weight: bold; margin-bottom: 2px;">
                        {{ $application->allotment->seat->floor }}F
                    </div>
                    <div style="margin-bottom: 2px;">
                        Room: {{ $application->allotment->seat->room_number }}
                    </div>
                    <div>
                        Bed: {{ $application->allotment->seat->bed_number }}
                    </div>
                </td>
                ...
                <td style="padding: 8px; border: 1px solid #000; text-align: center; font-size: 9px;">
                    @php
                        ...
                        $dept = $application->student->department ?? 'N/A';
                        $abbreviations = [
                    ...
                    @endphp
                    <span style="font-weight: bold; color: #2c3e50;">{{ $displayDept }}</span>
                </td>

                <!-- Semesters -->
                <td
                    style="padding: 8px; border: 1px solid #000; text-align: center; font-size: 9px; font-weight: bold;">
                    {{ $application->current_semesters }}
                </td>
                ...
                <td style="padding: 8px; border: 1px solid #000; text-align: center;">
                    <span
                        style="padding: 3px 6px; border-radius: 3px; font-size: 8px; font-weight: bold; text-transform: uppercase; 
                        @if ($application->status === 'pending') background: #fff3cd; color: #856404; 
                        @elseif($application->status === 'approved') background: #d4edda; color: #155724; 
                ...
                <!-- Date -->
                <td style="padding: 8px; border: 1px solid #000; text-align: center; font-size: 8px;">
    ...
    @if ($renewalApplications->isEmpty())
        <div style="text-align: center; padding: 40px; color: #7f8c8d; font-style: italic;">
            <h3>No Renewal Applications Found</h3>
            <p>There are currently no renewal applications to display in this report.</p>
        </div>
    @endif
    ...
    <div class="footer">
        ...
        This report contains {{ $renewalApplications->count() }} renewal applications | Generated:
        {{ now()->format('d M Y, H:i') }}<br>
        <em>Note: This report includes complete academic and personal information for administrative review</em>
    </div>
</body>
