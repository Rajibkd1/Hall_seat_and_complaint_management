class HallNoticeBoard {
    ...
    bindEvents() {
        ...
        document.addEventListener("keydown", (e) => {
            if (
                e.key === "Escape" &&
                !this.noticeModal.classList.contains("hidden")
            ) {
                this.closeNoticeModal();
            }
        });
    ...
    async loadMoreNotices() {
        ...
        try {
            ...
            const response = await fetch(
                `{{ route('student.hall-notice') }}?page=${
                    this.currentPage
                }&type=${this.currentFilter}&search=${encodeURIComponent(
                    searchQuery
                )}`,
                {
                    headers: {
    ...
    async openNoticeModal(noticeId) {
        try {
            const response = await fetch(
                `{{ route('student.hall-notice', '') }}/${noticeId}`,
                {
                    headers: {
                        "X-Requested-With": "XMLHttpRequest",
    ...
    populateModal(notice) {
        ...
        document.getElementById("modalCategory").textContent =
            notice.notice_type.charAt(0).toUpperCase() +
            notice.notice_type.slice(1);
        document.getElementById("modalContent").innerHTML = notice.description;
        document.getElementById("modalAuthor").textContent = notice.admin
            ? notice.admin.name
            : "Hall Administration";
    ...
    appendNotices(notices) {
        notices.forEach((notice, index) => {
            this.noticesContainer.appendChild(
                this.createNoticeCard(notice, index)
            );
        });
    }
    ...
    createNoticeCard(notice, index) {
        ...
        card.className =
            "notice-card bg-white/90 backdrop-blur-lg rounded-2xl p-6 border border-gray-200/50 cursor-pointer transition-all duration-300 hover:shadow-2xl shadow-lg animate-notice-appear relative group";
        ...
        card.innerHTML = `
                ...
                <div class="mb-4">
                    ...
                    <div class="text-gray-600 text-sm line-clamp-3 leading-relaxed rich-text-preview">${this.stripHtml(
                        notice.description
                    ).substring(0, 150)}${
            this.stripHtml(notice.description).length > 150 ? "..." : ""
        }</div>
                </div>
